...
class Puppy:
    ...
    def update_weight(self):
        if self.month < 48:
            self.weight = (self.month + 1) * 0.1
        else:
            self.weight = 3

    def get_n_month_old(self, n:int):
        self.month += n
        self.update_weight()
        return self.month, self.weight

    def get_n_year_old(self):
        pass

    def amounts(self):
        amount = round(self.weight * 1000 * 0.27, 2)
        print(f"{self.name}은(는) 하루에 {amount}ml의 분유를 먹습니다.")
        return amount
    ...
    def eat(self, g):
        ...
        self.eg += g
        if self.eg < amount:
            print(f"{self.name}은(는) 오늘 {self.eg}ml의 분유를 먹었습니다. 더 먹을 수 있어요!")
            self.full = False
        else:
            print(f"{self.name}은(는) 오늘 {self.eg}ml의 분유를 먹었습니다. 이제 배불러요!")
            self.full = True
        return self.eg, self.full
